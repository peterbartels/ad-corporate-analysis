...
**DEPT mapping:** Social icons  
**Used on:** 168 page(s)

Row of social share icons ('Share' label + Facebook, X, LinkedIn and email intents) shown on nearly all article and press-release pages. Share URL defaults to the canonical page URL.

> **Migration notes:** Good candidate for a global/template-level setting rather than per-page authoring.

## Example

![Social Share example — /en/home/company/reports/integrated-report](examples/social-share.png)

*Captured live from [/en/home/company/reports/integrated-report](https://www.averydennison.com/en/home/company/reports/integrated-report.html) — see the [page composition](../pages/en_home_company_reports_integrated-report/composition.md).*

## CMS data model

| Field | Type | Required | Description |
|---|---|---|---|
| `label` | string | no | Label before the icons, default 'Share'. |
| `networks` | array<string> | yes | Enabled networks, enum values: facebook | x | linkedin | email (merged: networks[] + platforms). |
| `shareUrl` | link | no | Override for the shared URL; defaults to the canonical page URL. |
