...
4. `image` — two executive portrait photos side by side in one row, each with overlaid name and title
   Margin below: ~40px (visual)
5. `social-share` — 'Share' with Facebook, X, LinkedIn, email icons between hairline rules
   Margin below: ~48px (visual)
6. `richtext` — body: dateline lead, appointment announcement, one bold-headed subsection per appointee with quote and bio, '###'
