...
4. `image` — announcement graphic combining the appointee's portrait with headline text
   Margin below: 50px (section-buffer)
5. `social-share` — 'Share' with Facebook, X, LinkedIn, email icons between hairline rules
   Margin below: ~48px (visual)
6. `richtext` — body: dateline lead, appointment copy, quote and bio paragraphs, '###'
   Margin below: 50px (section-buffer)
