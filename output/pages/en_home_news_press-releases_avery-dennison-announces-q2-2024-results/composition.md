...
4. `button` — 'Related' heading + black pill button to the Financial Review and Analysis PDF, in the sidebar
   Margin below: n/a (sidebar)
5. `image` — large quarter graphic (e.g. 'Q1 2024') as lead visual
   Margin below: 50px (section-buffer)
6. `social-share` — 'Share' with Facebook, X, LinkedIn, email icons between hairline rules
   Margin below: ~48px (visual)
7. `richtext` — complete earnings body: highlights bullets, dateline lead, quotes, segment results, guidance, '###'
   Margin below: 50px (section-buffer)
