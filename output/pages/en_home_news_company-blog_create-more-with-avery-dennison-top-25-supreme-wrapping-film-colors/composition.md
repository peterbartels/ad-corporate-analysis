...
**Page:** `/en/home/news/company-blog/create-more-with-avery-dennison-top-25-supreme-wrapping-film-colors`  
...
3. `image` — Hero product photo. *Margin below: ~40px (visual).*
4. `social-share` — 'Share' + icon row above a divider. *Margin below: ~64px (visual).*
5. `richtext` — Short copy block introducing the next product/color. Alternates with the image below — the pair repeats 5 times. *Margin below: ~40px (visual).*
6. `image` — Full-column product photo. *Margin below: ~40px (visual).*
7. `richtext` — Closing block: complete 25-item product list plus outro paragraph with links. *Margin below: ~40px (visual).*
