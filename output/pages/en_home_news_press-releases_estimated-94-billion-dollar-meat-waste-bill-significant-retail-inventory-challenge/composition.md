...
**Page:** `/en/home/news/press-releases/estimated-94-billion-dollar-meat-waste-bill-significant-retail-inventory-challenge`  
...
3. `media-contacts` — right-rail 'Media contact' with a single contact (name, mailto)
   Margin below: n/a (sidebar)
4. `image` — editorial photograph as lead visual
   Margin below: 50px (section-buffer)
5. `social-share` — 'Share' with Facebook, X, LinkedIn, email icons between hairline rules
   Margin below: ~48px (visual)
6. `richtext` — body: key-findings bullet list, dateline lead, research narrative with inline links, expert quotes and bold subheads
   Margin below: 50px (section-buffer)
7. `richtext` — 'Notes to editors': methodology and respondent-profile details, definitions
   Margin below: 50px (section-buffer)
8. `richtext` — 'About Avery Dennison' boilerplate
   Margin below: 50px (section-buffer)
