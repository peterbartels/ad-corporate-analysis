...
**Page:** `/en/home/technologies/creative-showcase`  
...
1. `richtext` — centered h1 title with a short centered intro paragraph. Margin below: 100px (content-buffer, 10rem).
2. `side-by-side` — repeated showcase card, ~6 instances stacked vertically (white card with drop shadow): heading, description paragraph, black "Learn more" pill button on the left, image on the right; some cards append a "Regional sites available" inline link row. Margin between cards: 50px (section-buffer, 5rem); after the last card: 100px (content-buffer) to the footer.
