...
**Page:** `/en/home/news/company-blog/building-markets-an-adf-grantee`  
...
2. `article-header` — H1 headline, author byline (e.g. foundation team) and date — no standfirst, no hero. *Margin below: ~24px (visual).*
3. `social-share` — 'Share' + icon row directly below the byline. *Margin below: 50px (section-buffer).*
4. `richtext` — Body copy block (intro / program description / quotes). *Margin below: ~40px (visual).*
5. `image` — Captioned photo (caption may include a quote and attribution). Alternates with the richtext above — the richtext/image pair repeats 5 times through the article. *Margin below: ~40px (visual).*
6. `richtext` — Closing paragraphs. *Margin below: 50px (section-buffer).*
7. `icon-button` — Repeated '← Back to all blogs' link. *Margin below: 100px (content-buffer).*
